...
impl<'a> Icon<'a> {
    ...
    pub fn draw_scaled(&self, display: &mut UefiDisplay, x: i32, y: i32, target_size: u32) {
        ...
        for ty in 0..target_size {
            for tx in 0..target_size {
                ...
                let x0 = if src_x >= 0.0 { src_x as i32 } else { (src_x - 1.0) as i32 };
                let y0 = if src_y >= 0.0 { src_y as i32 } else { (src_y - 1.0) as i32 };
                ...
                for dy in -1..=2 {
                    for dx in -1..=2 {
                        ...
                        if idx + 3 >= self.data.len() { continue; }
                ...
                if weight_sum > 0.0 {
                    ...
                    if a > 1 {
                        ...
                        let _ = Pixel(Point::new(x + tx as i32, y + ty as i32), blended).draw(display);
                    }
                }
            }
        }
    ...
    pub fn draw(&self, display: &mut UefiDisplay, x: i32, y: i32) {
        self.draw_scaled(display, x, y, 64);
    }
}
